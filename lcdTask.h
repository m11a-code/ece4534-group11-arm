...
/*
* Send ADC bytes to the LCD task for it to display as a waveform
* Args:
*    lcdData -- a pointer to a variable of type vtLCDStruct
*    length -- number of bytes in the array -- the call will result in a fatal error if you exceed the maximum length
*    pString -- string to print to the LCD
*    ticksToBlock -- how long the routine should wait if the queue is full
*  Return:
*    Result of the call to xQueueSend()
