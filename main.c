...
#define mainCONTROL_TASK_PRIORITY			( tskIDLE_PRIORITY)
...
/* ======= DATA STRUCTURES ========= */
static CalcStruct calcData;
static MappingStruct mapData;
static UARTStruct uartData;
static WebStruct webData;
static ControlStruct controlData;
...
int main( void )
{
	...
	vStartCalcTask(&calcData, mainCALC_TASK_PRIORITY, 	&mapData, &webData);
	vStartControlTask(&controlData, mainCONTROL_TASK_PRIORITY, &uartData, &webData);
	vStartMappingTask(&mapData, mainMAPPING_TASK_PRIORITY, &webData);
	vStartUARTTask(&uartData, mainUART_TASK_PRIORITY, &calcData, &controlData);
	vStartWebServerTask(&webData, mainWEBSERVER_TASK_PRIORITY, &controlData);
	...
	vStartConductorTask(&conductorData,mainCONDUCTOR_TASK_PRIORITY,&vtI2C0,&tempSensorData, &calcData, &controlData, &mapData, &uartData, &webData);
	#endif
