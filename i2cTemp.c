...
portBASE_TYPE SendTempValueMsg(vtTempStruct *tempData,uint8_t msgType,uint8_t* value,portTickType ticksToBlock, uint8_t rxLen)
{
	vtTempMsg tempBuffer;
	...
	tempBuffer.length = rxLen;
	if (tempBuffer.length > vtTempMaxLen) {
		// no room for this message
		VT_HANDLE_FATAL_ERROR(tempBuffer.length);
	}
	// instead of sizeof(value); try 2.
	//memcpy(tempBuffer.buf,(char *)&value,sizeof(value));	 @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	memcpy(tempBuffer.buf,(char *)value, rxLen);
...
uint8_t* getPointer(vtTempMsg *Buffer)
{
	uint8_t *ptr = (uint8_t *) Buffer->buf;
	return(ptr);
}
...
static portTASK_FUNCTION( vi2cTempUpdateTask, pvParameters )
{
	...
	for(;;)
	{
		...
		case vtI2CMsgTypeTempRead1: {
			...
			{ 
				uint8_t* recValues = getPointer(&msgBuffer);
				sprintf(lcdBuffer,"Received: %X %X", msgBuffer.buf[0], msgBuffer.buf[1]);
				if (lcdData != NULL) 
				{

					if (SendLCDadcMsg(lcdData, msgBuffer.length, msgBuffer.buf,portMAX_DELAY) != pdTRUE) 
					{
						VT_HANDLE_FATAL_ERROR(0);
					}
